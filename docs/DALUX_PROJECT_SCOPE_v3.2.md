...
8. **CS053 category-header rows absent on some forms** — `DLX_2_form_udfs` rows like `"1. Safe Access/Egress..."` (parent-category headers) are missing entirely on some forms rather than just having NULL `value_text`. First seen on NESY form `S405358071733291008`: only 2 of 14 category headers present (categories 2 and 6 — the two the inspector ticked a parent state for). Cause unclear: either an older Dalux template version on that project, or the n8n sync dropping rows with no inspector-set state. Workaround: `cs053.py` queries category-title rows separately (regex, any `value_text`), and falls back to a hardcoded `CS053_CATEGORY_TITLES` map for categories whose rows are missing entirely. Dev team should investigate whether the sync is dropping rows vs whether the form template itself varies across projects — if the latter, the hardcoded title list will need a per-template-version map.

9. **Site name / project number do NOT come from form UDFs** — Dalux auto-populates these from project metadata; they aren't inspector-filled fields. Early cs053.py drafts looked them up via `Project Name` / `Project Number` UDF rows, which happened to work for Carrington (whose sync evidently includes them as reference-like rows) but not for NESY (which doesn't). Correct source is `sheq_sites` (preferred, master list) with fallback to `DLX_2_projects.projectName` / `DLX_2_projects.number` for unmapped Dalux projects. Implemented in `build_payload` as a dedicated join rather than reading UDFs.
